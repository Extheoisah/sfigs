import Image from "next/image"
import { auth } from "./auth"

export default async function Home() {
    const session = await auth()
    return (
        <main className="flex min-h-screen flex-col items-center justify-between p-4 md:p-24">
            <section className="z-10 max-w-5xl w-full md:w-2/4 flex flex-col items-center justify-between font-mono text-sm">
                <div className="flex w-full flex-col md:flex-row items-center justify-between border-b border-gray-300 bg-gradient-to-b from-zinc-200 py-4 backdrop-blur-2xl dark:border-neutral-800 dark:from-inherit rounded-xl border bg-gray-200 p-4 dark:bg-zinc-800/30">
                    <p className="w-full mb-4 md:mb-0">
                        Welcome {session?.user.name}!
                    </p>
                    <picture className="flex w-full md:w-auto items-end justify-end">
                        <Image
                            src={session?.user.image || "/vercel.svg"}
                            alt="Vercel Logo"
                            className="rounded-full"
                            width={48}
                            height={48}
                            priority
                        />
                    </picture>
                </div>
                <div className="text-center">
                    <h1 className="text-2xl md:text-4xl font-bold mt-10">
                        Surfacing Good First Issues
                    </h1>
                    <p className="mt-4">for Open Source Projects</p>
                </div>
                <footer className="flex w-full items-center justify-center mt-10 border-t border-gray-300 bg-gradient-to-t from-zinc-200 py-4 backdrop-blur-2xl dark:border-neutral-800 dark:from-inherit rounded-xl border bg-gray-200 p-4 dark:bg-zinc-800/30">
                    <a
                        href="https://builders.btrust.tech/"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="flex flex-col md:flex-row items-center justify-between"
                    >
                        <p className="text-sm">Made with ❤️ by</p>
                        <picture className="flex items-end justify-end mt-2 md:mt-0 md:ml-2">
                            <Image
                                src="data:image/png;base64,"
                                alt="Btrust builders Logo"
                                className="rounded-full"
                                width={48}
                                height={48}
                                priority
                            />
                        </picture>
                    </a>
                </footer>
            </section>
        </main>
    )
}
